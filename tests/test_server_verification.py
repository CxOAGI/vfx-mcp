...
# Tools currently registered by the VFX MCP server.
...
class TestServerVerification:
    ...
    async def test_all_tools_registered(self, mcp_server: FastMCP[None]) -> None:
        ...
        async with Client(mcp_server) as client:
            ...
            expected_format_tools = {"convert_format", "batch_convert"}
    ...
    async def test_resource_endpoints(self, mcp_server: FastMCP[None]) -> None:
        ...
        async with Client(mcp_server) as client:
            ...
            # Templated resources (metadata endpoint is a URI template)
            templates = await client.list_resource_templates()
            template_uris = {str(template.uriTemplate) for template in templates}
            assert any(
                uri.startswith("videos://") and uri.endswith("/metadata")
    ...
    async def test_end_to_end_workflow(
        ...
        async with Client(mcp_server) as client:
            ...
            video_info = json.loads(info_result[0].text)
            assert "duration" in video_info
            assert "video" in video_info
            assert "width" in video_info["video"]
            assert "height" in video_info["video"]
    ...
    async def test_error_handling(
        ...
        async with Client(mcp_server) as client:
            ...
            # Test with non-existent input and invalid output path
    ...
    async def test_performance_benchmarks(
        ...
        async with Client(mcp_server) as client:
            ...
            assert thumb_time < 10.0, (
                f"Thumbnail generation took too long: {thumb_time}s"
            )
    ...
    async def test_resource_file_discovery(
        ...
        async with Client(mcp_server) as client:
            ...
            try:
                ...
                content = list_response[0].text
                assert isinstance(content, str)

                # Content should be JSON with a video file list
    ...
    async def test_concurrent_operations(
        ...
        async with Client(mcp_server) as client:
            ...
            tasks = [process_video(video, i) for i, video in enumerate(sample_videos)]

            results = await asyncio.gather(*tasks, return_exceptions=True)

            success_count = sum(1 for result in results if result is True)
            assert success_count == len(sample_videos), (
                f"Only {success_count}/{len(sample_videos)} concurrent "
