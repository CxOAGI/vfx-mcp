"""Comprehensive server verification tests for VFX MCP - Fixed Version.
...
class TestServerVerification:
    ...
    async def test_resource_endpoints(self, mcp_server: FastMCP[None]) -> None:
        ...
        async with Client(mcp_server) as client:
            ...
            # Check for expected static resource endpoints
            assert "videos://list" in resource_uris

            # The per-file metadata endpoint is a resource template
    ...
    async def test_end_to_end_workflow(
        ...
        async with Client(mcp_server) as client:
            ...
            info_result = await client.call_tool(
                "get_video_info", {"video_path": str(sample_video)}
            )
            ...
            await client.call_tool(
                "extract_audio",
                {"input_path": str(sample_video), "output_path": str(audio_output)},
            )
    ...
    async def test_error_handling(
        ...
        async with Client(mcp_server) as client:
            ...
            try:
                await client.call_tool(
                    "get_video_info", {"video_path": str(nonexistent_file)}
                )
