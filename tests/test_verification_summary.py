...
class TestVerificationSummary:
    ...
    @pytest.mark.integration
    async def test_tool_categories_verification(
        ...
        async with Client(mcp_server) as client:
            ...
            print(
                f"Basic Video Tools: {len(basic_present)}/{len(basic_tools)} - {basic_present}"
            )
            ...
            print(
                f"Audio Tools: {len(audio_present)}/{len(audio_tools)} - {audio_present}"
            )
            ...
            print(
                f"Effects Tools: {len(effects_present)}/{len(effects_tools)} - {effects_present}"
            )
            ...
            print(
                f"Format Tools: {len(format_present)}/{len(format_tools)} - {format_present}"
            )
    ...
    @pytest.mark.integration
    async def test_resource_endpoints_verification(
        self, mcp_server: FastMCP[None]
    ) -> None:
        ...
        async with Client(mcp_server) as client:
            ...
            print("Note: Metadata resources are created dynamically for video files")
